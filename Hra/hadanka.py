class Hadanka():
    ...
    def ziskej_cislo_hrace(self):
        ...
        while True:
            try:
                self.cislo_hrace = int(input(f"Zadej číslo v rozsahu {self.min_cislo} až {self.max_cislo}"))
                if self.cislo_hrace in range(self.min_cislo, self.max_cislo + 1):
                    return self.cislo_hrace
                else:
                    print(f"Špatně! Zadej prosím číslo v rozsahu {self.min_cislo} až {self.max_cislo}")
            except ValueError:
